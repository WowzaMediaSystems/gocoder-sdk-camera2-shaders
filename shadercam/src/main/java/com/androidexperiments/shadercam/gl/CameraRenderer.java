...
public class CameraRenderer extends Thread implements SurfaceTexture.OnFrameAvailableListener
{
    ...
    protected void encodeFrame(SurfaceTexture surfaceTexture) {
        if (mIsRecording)
            dupSurfaceContents(surfaceTexture, mWindowSurface, mRecordSurface, true);
    }
    ...
    protected void dupSurfaceContents(SurfaceTexture surfaceTexture,
                                      WindowSurface srcSurface, WindowSurface destSurface,
                                      boolean swapBuffers) {

        if(mEglCore.getGlVersion() >= 3)
        {
            destSurface.makeCurrentReadFrom(srcSurface);

            GlUtil.checkGlError("before glBlitFramebuffer");

            GLES30.glBlitFramebuffer(
                    0, 0, srcSurface.getWidth(), srcSurface.getHeight(),
                    0, 0, destSurface.getWidth(), destSurface.getHeight(), //must match the mediarecorder surface size
        ...
        else //gl v2
        {
            ...
            destSurface.makeCurrent();

            setViewport(destSurface.getWidth(), destSurface.getHeight());
            draw();
            setViewport(srcSurface.getWidth(), srcSurface.getHeight());
        }

        if (swapBuffers) {
            destSurface.setPresentationTime(surfaceTexture.getTimestamp());
            destSurface.swapBuffers();
        }
    }
    ...
    protected EglCore getEglCore() {
        return mEglCore;
    }

    protected WindowSurface getPreviewSurface() {
